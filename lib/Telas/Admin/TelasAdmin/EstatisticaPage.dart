...
class _EstatisticaPageState extends State<EstatisticaPage> {
  ...
  Widget build(BuildContext context) {
    if (estController == null) {
      estController = new EstatisticaController(widget.campanha);
    }
    // TODO: implement build
    return Scaffold(
      appBar: myAppBar('Estatística', context),
      body: Container(
        child: StreamBuilder(
            ...
            builder: (context, snapshot) {
              ...
              return Column(
                children: <Widget>[getEstatisticasWidget(snapshot.data)],
              );
            }),
      ),
    );
  ...
  getEstatisticasWidget(List<Corrida> corridas) {
    ...
    for (Corrida c in corridas) {
      ...
      for (String s in carroIds) {
        if (s == c.id_carro) {
          containsCarro = true;
        }
      }
      if (!containsCarro) {
        carroIds.add(c.id_carro);
      }
    }
    int carros = carroIds.length;
    return hText(
        'Total de visualizações: ${visualizacoes} dist: ${dist} Carros Participantes:${carros}',
        context);
  }
